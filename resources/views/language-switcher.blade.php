...
<x-filament::dropdown placement="bottom-start">
    <x-slot name="trigger">
        @if (isset($currentLanguage) && $showFlags)
            <div class="pl-2">
                <div class="flex items-center text-sm">
                    <span class="mr-3 size-7.5"
                        style="width: 30px;">{{ try_svg('flag-1x1-' . $currentLanguage['flag'], 'rounded-full w-8 h-8') }}</span>
                    @if ($isInSideBar)
        ...
        @else
            <x-filament::icon-button icon="heroicon-o-language" label="New label" />
        @endif
    </x-slot>
